...
const Quiz = () => {
      ...
      <div className="max-w-3xl mx-auto space-y-6 sm:space-y-8 py-6 sm:py-10">
        <div className="flex items-center justify-between gap-4">
          <div className="space-y-1">
            <h2 className="text-lg sm:text-xl font-black text-slate-900 tracking-tight leading-tight">Kuis Berjalan</h2>
            <p className="text-[10px] font-bold text-indigo-600 uppercase tracking-[0.1em]">Soal {currentIndex + 1} / {questions.length}</p>
          </div>
          <div className={`flex items-center gap-2 px-3 sm:px-4 py-2 rounded-2xl font-black text-xs sm:text-sm ${timeLeft < 60 ? 'bg-rose-500 animate-pulse' : 'bg-slate-900'} text-white shrink-0`}>
            <Timer className={`w-4 h-4 ${timeLeft < 60 ? 'text-white' : 'text-amber-400'}`} /> {formatTime(timeLeft)}
          </div>
        </div>

        <Progress value={((currentIndex + 1) / questions.length) * 100} className="h-2 rounded-full bg-slate-100" />

        <Card className="p-5 sm:p-8 space-y-6 sm:space-y-8 shadow-2xl shadow-indigo-100/50">
          <p className="text-xl sm:text-2xl font-black text-slate-800 leading-tight">{q.question_text}</p>
          <div className="grid grid-cols-1 gap-3 sm:gap-4">
            {q.choices.map((choice: any) => (
              <button 
                key={choice.id} 
                onClick={() => handleAnswer(choice.id)}
                className={`p-4 sm:p-6 rounded-2xl border-2 text-left font-black transition-all transform active:scale-95 ${
              ...
              >
                <div className="flex items-center gap-3 sm:gap-4">
                  <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center border-2 shrink-0 ${
          ...
          <Button 
            ...
            className="w-full h-12 sm:h-14 text-base sm:text-lg font-black shadow-xl shadow-indigo-200"
          >
  ...
  return (
    <div className="max-w-2xl mx-auto py-10 sm:py-20 text-center space-y-10">
      <div className="relative inline-block">
        <div className="absolute inset-0 bg-indigo-500 blur-3xl opacity-20 animate-pulse" />
        <Card className="p-8 sm:p-12 relative z-10 space-y-6 rounded-[48px] border-none shadow-2xl shadow-indigo-200">
          <div className="w-20 h-20 sm:w-24 sm:h-24 bg-indigo-600 text-white rounded-[24px] sm:rounded-[32px] flex items-center justify-center mx-auto shadow-xl">
            <Award className="w-10 h-10 sm:w-12 sm:h-12" />
          </div>
          <div className="space-y-2">
            <h2 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">Skor Akhir Kamu</h2>
            <p className="text-5xl sm:text-6xl font-black text-indigo-600">{score}%</p>
          </div>
          <div className="flex justify-center gap-4 sm:gap-8 py-4 border-y border-slate-100">
            <div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Benar</p>
              <p className="text-lg sm:text-xl font-black text-emerald-500">{(score / 100) * questions.length}</p>
            </div>
            <div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">XP Diperoleh</p>
              <p className="text-lg sm:text-xl font-black text-amber-500">+{score * 10}</p>
            </div>
          </div>
          <div className="flex flex-col sm:flex-row gap-3 pt-4">
            <Button onClick={() => setStep('selection')} variant="outline" className="flex-1 h-12 rounded-2xl">Main Lagi</Button>
            <Button onClick={() => navigate('/dashboard')} className="flex-1 h-12 rounded-2xl">Ke Dashboard</Button>
          </div>
        </Card>
      </div>
    </div>
